 ...
 * @retval  
 */
void kalman1_param_init(kalman1_state *state)
{
    state->x = 1;
    state->p = 5e2;
    state->A = 1;
    ...
    float init_x, float init_p)
{
		...
		state->kalman1_param_init = kalman1_param_init;
		state->kalman1_filter = kalman1_filter;
	
		state->kalman1_param_init(state);
}
 ...
 * @retval  
 */
void kalman2_param_init(kalman2_state *state ,float A_0_1 )
{
...
void kalman2_struct_init( kalman2_state *state, float A_0_1 )
{
		...
		state->kalman2_param_init = kalman2_param_init;
		state->kalman2_filter = kalman2_filter;
	
		state->kalman2_param_init(state ,A_0_1 );
}
